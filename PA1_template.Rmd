...
We should also change interval data as it has form *hhmm* instead of just representing actual time of the date. You can see it from summary statistics, where max value for `interval` is `r max(data$interval)`. 
...
The graph shows us that the day interval when people are the most active is `r format(max_interval,"%H:%M")`. On average, people went `r comma_format(accuracy = 0.01)(max_value)` steps during that interval.
